...
def proj(pcd, img_path, mask_path, proj_dict, instance_id):
    ...
    for idx, mask in enumerate(masks):
        ...
        points = tmp[labels == most_frequent_label]
...
def process_cam(cam1, cam1_pcd_dict, pcd_img_dict, SAM_path, pcl_path, img_path, label_idx, color_idx, label_inverse):
    ...
    os.makedirs(join('/lab/tmpig13d/henghui/inss', pcd_name[:-4]), exist_ok=True)
    label_idx, color_idx, label_inverse = write_ply_point_cloud(join('/lab/tmpig13d/henghui/inss', pcd_name[:-4], pcd_name), pcd, intensity, proj_dict, label_idx, color_idx, label_inverse)

    for img_name in pcd_img_dict[pcd_name]:
        shutil.copy(join(img_path, img_name), join('/lab/tmpig13d/henghui/inss', pcd_name[:-4], img_name))
...
def run(folder_path):
    ...
    for cam1 in CAM1_res:
        thread = threading.Thread(target=process_cam, args=(cam1, cam1_pcd_dict, pcd_img_dict, SAM_path, pcl_path, img_path, label_idx, color_idx, label_inverse))
        thread.start()
